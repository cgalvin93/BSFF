...
#cluster_results (default from cluster_contacts.py)
#time ipython ~/desktop/prj/bsff/bsff_scripts/fuzzball_for_assembly.py a8s
...
cluster_dir_paths=['/wynton/home/kortemme/cgalvin/round6/a8s/a8sfragfbs/a8s_Fragment_1_residue_contacts']
params_directory_path='/wynton/home/kortemme/cgalvin/round6/a8s/Inputs/Rosetta_Inputs/'
params_file_path=[]
for i in os.listdir(params_directory_path):
    if i[-7:]=='.params':
        params_file_path.append(os.path.join(params_directory_path,i))
# cluster_dir_paths=[]
# for i in os.listdir(TAPDBS_path):
#     if os.path.exists(os.path.join(TAPDBS_path,i)) is True and i[-3:]=='pdb':
#         cluster_path=os.path.join(TAPDBS_path,i)
...
for cluster_dir in cluster_dir_paths: ############################
    try:
        pdbfiles=[i for i in os.listdir(cluster_dir) if i[-3:]=='pdb']
    except:
        continue
...
json.dump(fuzzball_pose_frequencies,open('fuzzball_for_assembly_frequencies.json','w'))
print('\n')
print('\n')
print('\n')
print('\n')

print('THE NUMBER OF FUZZBALL CONTACTS IS: '+str(n_fuzzball_residues))
