...
#m is number residues in each motif
import argparse
...
parser.add_argument('-m','--n_motif_res', help='number of desired residues in motifs, default=3', required=False)
#
...
if args['n_motif_res']:
    nmotifres=int(args['n_motif_res'])
else:
    nmotifres=3
...
fuzzball=os.path.join(parent_directory,'clean_fuzzball_for_assembly_np.pdb')
freqjson=os.path.join(parent_directory,'np_fuzzball_for_assembly_frequencies.json')
...
def init_random_motif():
    motif_pose=Pose()
    res_in_motif=[]
    while len(res_in_motif)<nmotifres:
...
for z in range(n_trajectories):
    ...
    for kT in temperatures:
        for trial in range(n_trials):
            ...
            random_position=random.choice([xj+1 for xj in range(nmotifres)])
            ...
            if trial_score<initial_score:
                ...
                if nmotifres==2:
                    all_accepted.append((res_in_motif[0],res_in_motif[1],weighted_score))
                elif nmotifres==3:
                    all_accepted.append((res_in_motif[0],res_in_motif[1],res_in_motif[2],weighted_score))
                elif nmotifres==4:
                    all_accepted.append((res_in_motif[0],res_in_motif[1],res_in_motif[2],res_in_motif[3],weighted_score))
                e_over_traj.append(weighted_score)
                accepted_seqs.append((motif_pose.sequence()[:nmotifres],weighted_score))
            else:
...
saccepted=sorted(all_accepted,key=lambda val: val[-1])
solnsonlyres=[]
solnsonlyE=[]
if nmotifres==2:
    for i in saccepted:
        solnsonlyres.append((i[0],i[1]))
        solnsonlyE.append(i[-1])
elif nmotifres==3:
    for i in saccepted:
    ...
    for i in saccepted:
        solnsonlyres.append((i[0],i[1],i[2],i[3]))
        solnsonlyE.append(i[-1])
...
def output_motif(residues):
    ...
    # sf.show(motif)
    mnstrs=[str(i) for i in motif_res]
    motif_name='_'.join(mnstrs)
...
for resname in amino_acids:
    ...
    for s,tscore in accepted_seqs:
        for i in s:
            if i==aa:
                count+=1
    aadict[resname]=count/(len(accepted_seqs)*nmotifres)
    sanitylist.append(count/(len(accepted_seqs)*nmotifres))
...
for resname in amino_acids:
    ...
    for s,score in select_seqs:
        for i in s:
            if i==aa:
                count+=1
    aadict[resname]=count/(len(select_seqs)*nmotifres)
    sanitylist.append(count/(len(select_seqs)*nmotifres))
# print(str(sum(sanitylist)))
scx,scy=list(aadict.keys()),aadict.values()
sortlist=[]
for i,e in enumerate(scy):
...
total_res=float(len(accepted_seqs)*nmotifres)
