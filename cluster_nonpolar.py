...
polar_residues=['TYR','TRP','LEU','ILE','VAL','PHE','MET'] #its actually np residues lol no gly/ala/pro; tyr included 
fragdata={}
for frag in fraglist:
    ...
    for clust in clusts:
        ...
        if restype in polar_residues:
            polfiles.append(os.path.join(clusters_path,clust))
        else:
            pass
    fragdata[frag]=polfiles


pdfname=str(parent_directory)+'_cluster_statistics.pdf'
pdf = PdfPages(pdfname)

for keyf in fragdata.keys():
    for rfb in fragdata[keyf]:
        df=pdb_to_df(rfb)
        ...
        print('starting to cluster contacts...')
        print(str(keyf))
        print(str(rfb))
        ...
        newdir=rfb.split('.')[0]+'_clustered'
        cluster_results_path=os.path.join(fragfbdir,frag,newdir)
