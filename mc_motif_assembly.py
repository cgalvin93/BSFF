...
parser.add_argument('-w','--stat_weight', help='weight of statistical term', required=False)
...
if args['stat_weight']:
    stat_weight=float(args['stat_weight'])
else:
    stat_weight=1.0
...
'''
made way solutions list is appended diff
    doesnt include last from each t
    instad of while loop just appends from solnsonlyres[:naddmotifs]
    additional checks in output motif function to make sure it isnt generating billions
        check if file already exists
        check that number of files created is not greater than desired n addnl motifs

let's try to test this with a small number of trajectories but the desied 1k trials per traj
...
time python ~/bsff_scripts/fixmc.py -d '$compound_dir' -w 1.0 -t 100 -r 1000 -n 1000 -p '$compound_dir'_fixmctest1ktraj
qstat -j "$JOB_ID"
'>fixmctest$compound_dir.sh

chmod ugo+x fixmctest$compound_dir.sh
qsub -cwd -l mem_free=16G -o cluster_output -e cluster_output fixmctest$compound_dir.sh

ayyy it worked
    ordered e dist, change y axis to like -50 to 50
    energy over traj needs similar y axis change, + is showing only 2k steps???
        makes sense actually, its only accepted moves

-------> make fixes
-------> test if works for other targets
-------> if so increase n traj
-------> if that looks good, implement cmd line args for pyrosetta weights, repeat original plan
...
source ~/anaconda3/etc/profile.d/conda.sh
conda activate pyr37
time python ~/bsff_scripts/fixmc.py -d '$compound_dir' -w 1.0 -t 500 -r 1000 -n 1000 -p '$compound_dir'_fixmctest500traj
qstat -j "$JOB_ID"
'>fixmctest$compound_dir.sh
...
(base) [cgalvin@log2 drug_targets]$ beegfs-ctl --getquota --storagepoolid=11 --uid cgalvin
      user/group     ||           size          ||    chunk files
     name     |  id  ||    used    |    hard    ||  used   |  hard
--------------|------||------------|------------||---------|---------
       cgalvin| 61046||  731.45 GiB| 1000.00 GiB|| 15852569|unlimited


scp -r cgalvin@log2.wynton.ucsf.edu:drug_targets/a8s_a8s_fixmctest500traj_mc_stats.pdf ~/desktop/a8s.pdf
...
(base) [cgalvin@log2 ~]$ beegfs-ctl --getquota --storagepoolid=11 --uid cgalvin
     ...
     name     |  id  ||    used    |    hard    ||  used   |  hard
--------------|------||------------|------------||---------|---------
       cgalvin| 61046||  702.21 GiB| 1000.00 GiB|| 13172662|unlimited




okay could make it so e over traj just shows few examples
select top 10k for res stat, unique res analysis
play with weights and stuff

okay, i think i implemented this, lets give it a try and see if it works
...
source ~/anaconda3/etc/profile.d/conda.sh
conda activate pyr37
time python ~/bsff_scripts/fixmc.py -d '$compound_dir' -w 1.0 -t 1000 -r 1000 -n 1000 -p '$compound_dir'_fixmctest1ktraj -a 1.0 -b 0.55 -c 1.0 -e 0.5 -f 2.0 -g 2.0
qstat -j "$JOB_ID"
'>fixmctest1k_2$compound_dir.sh

chmod ugo+x fixmctest1k_2$compound_dir.sh
qsub -cwd -l mem_free=16G -o cluster_output -e cluster_output fixmctest1k_2$compound_dir.sh
...
annnnd okay, it appears to have worked :)
hist plot doesnt look good, cant just set xlim. need to either prune data or
set bins manually or something
    actually just gonna comment it out, really the second plot is more important to me



on the 26th, the rmcr is finally finished
(base) [cgalvin@log2 ~]$ beegfs-ctl --getquota --storagepoolid=11 --uid cgalvin
      user/group     ||           size          ||    chunk files
     name     |  id  ||    used    |    hard    ||  used   |  hard
--------------|------||------------|------------||---------|---------
       cgalvin| 61046||  611.61 GiB| 1000.00 GiB||  4850708|unlimited
